            ...
            >
              <table class="min-w-full divide-y divide-gray-200" >
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                  <tr v-for="item in indexKpistatuses.data.data" :key="item.id">
                    <td
              ...
              <IndexPagination :index="indexKpistatuses" />
            </div>
          </div>
        </div>
      </div>
...
<script setup lang="ts">
  ...
  import Form from "./forms/StatusForm.vue";
  import { PencilSquareIcon, TrashIcon } from '@heroicons/vue/24/outline'
  const indexKpistatuses = useIndexStore('statuses')(),
    processing = ref(true)
  // invitations = ref([])

  checkProcessing()

  indexKpistatuses.setConfig({
    uri: 'statuses',
    orderByDirection: 'desc',
  })
  indexKpistatuses.fetch()


  function checkProcessing() {
    setTimeout(function () {
      if (indexKpistatuses.fetching ) {
        checkProcessing()
        return
      }
      renderData()
